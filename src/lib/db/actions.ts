...
export async function getHeroData(lang: Language) {
  return await db.query.heroSections.findFirst({
...
export async function createAdvantage(lang: Language, advantage: Omit<Advantage, 'id'>) {
  const [newAdvantage] = await db
    .insert(schema.advantages)
    .values({ ...advantage, lang })
    .returning();
  return newAdvantage;
}
...
export async function getActionSectionData(lang: Language) {
  return await db.query.actionSections.findFirst({
  ...
  try {
    ...
    if (currentSlugs.length > 0) {
      await db
        ...
        .where(
          and(eq(schema.creators.lang, lang), notInArray(schema.creators.slug, currentSlugs))
        );
    } else {
...
export async function getDevNoteBySlug(slug: string) {
  return await db.query.devNotes.findFirst({
    where: eq(schema.devNotes.slug, slug)
  });
}
