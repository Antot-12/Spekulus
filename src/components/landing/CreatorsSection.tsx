...
export function CreatorsSection() {
  ...
  const loadCreators = useCallback(async () => {
    ...
    try {
      ...
      // Sort creators by ID
      const sortedCreators = visibleCreators.sort((a, b) => a.id - b.id);
    ...
    <section id="creators" className="py-16 md:py-24 bg-muted/20">
      <div className="container mx-auto px-4">
        ...
        {!isLoaded ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-y-10 gap-x-6 justify-items-center">
              {creators.map((creator, index) => (
                <div 
                  key={creator.id}
                  className="group relative flex flex-col items-center text-center opacity-0 animate-fade-in-up w-full max-w-xs"
                    ...
                    <div className="relative h-40 w-40">
                      {creator.imageId && (
                        <Image 
                            ...
                            layout="fill" 
                            objectFit="cover" 
                            className="rounded-full group-hover:scale-105 transition-transform duration-300 shadow-lg group-hover:shadow-primary/20"
