...
import { Trash2, PlusCircle, Save, Loader2, Sparkles, Coffee, Thermometer, Zap, Home } from 'lucide-react';
...
export default function ScenariosAdminPage() {
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            await updateScenarios(selectedLang, allData[selectedLang]);
            toast({ title: "Saved!", description: `All scenario changes for ${languageNames[selectedLang]} have been saved.`});
            logAction('Scenarios Update', 'Success', `Saved all changes for ${languageNames[selectedLang]} scenarios.`);
        } catch (error) {
    ...
    const handleScenarioAdd = async () => {
        ...
        try {
            const newScenario = await createScenario(selectedLang, newScenarioData);
            if (newScenario) {
    ...
    const handleScenarioDelete = (id: number) => {
        const itemToDelete = scenarios.find(item => item.id === id);
        ...
        logAction('Scenarios Update', 'Success', `Deleted scenario "${itemToDelete?.question}" for ${languageNames[selectedLang]}.`);
    };
