...
export default function FaqAdminPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateFaqs(selectedLang, cleaned)
      toast({ title: 'Saved!', description: `All FAQ changes for ${languageNames[selectedLang]} saved.` })
      logAction('FAQ Update', 'Success', `Saved changes for ${languageNames[selectedLang]}`)
    } catch {
  ...
  const handleAdd = async () => {
    try {
      const newFaq = await createFaq(selectedLang, { question: 'New Question?', answer: 'New Answer.' })
      if (newFaq) {
