                        ...
                        <div key={creator.id} className="space-y-6 p-4 border rounded-md">
                            ...
                            <Card><CardHeader><CardTitle className="font-headline flex items-center gap-2"><Heart className="w-6 h-6"/>Personality &amp; Details</CardTitle></CardHeader>
                                <CardContent className="space-y-4">
                                    ...
                                    <div className="space-y-2"><Label htmlFor={`skills-${creator.id}`}>Skills (comma-separated)</Label><Input id={`skills-${creator.id}`} value={creator.skills?.join(', ') ?? ''} onChange={(e) => handleArrayChange(id, 'skills', e.target.value)} /></div>
                                    <div className="space-y-2"><Label htmlFor={`languages-${creator.id}`}>Languages (comma-separated)</Label><Input id={`languages-${creator.id}`} value={creator.languages?.join(', ') ?? ''} onChange={(e) => handleArrayChange(id, 'languages', e.target.value)} /></div>
                                    <div className="space-y-2"><Label htmlFor={`contributions-${creator.id}`}>Contributions (comma-separated)</Label><Textarea id={`contributions-${creator.id}`} value={creator.contributions?.join(', ') ?? ''} onChange={(e) => handleArrayChange(id, 'contributions', e.target.value)} rows={3}/></div>
                                    <div className="space-y-2"><Label htmlFor={`hobbies-${creator.id}`}>Hobbies (comma-separated)</Label><Input id={`hobbies-${creator.id}`} value={creator.hobbies?.join(', ') ?? ''} onChange={(e) => handleArrayChange(id, 'hobbies', e.target.value)} /></div>
                                </CardContent>
                            </Card>
